...
def log_step(message: str, type: str = "info"):
    ...
    print(
        f"{Fore.WHITE}[{timestamp}] {color}{prefix.get(type, '•')} {message}{Style.RESET_ALL}"
    )
...
class ServiceCapmonster:
    ...
    async def get_captcha_token_async(self):
        task_id = self.capmonster.create_task(
            website_key=CaptchaConfig.WEBSITE_KEY, website_url=CaptchaConfig.WEBSITE_URL
        )
    ...
    async def get_captcha_token_async(self):
        result = await asyncio.to_thread(
            lambda: self.solver.turnstile(
                sitekey=CaptchaConfig.WEBSITE_KEY, url=CaptchaConfig.WEBSITE_URL
            )
        )
        return result["code"]
...
class ProxyManager:
    def __init__(self, proxy_list: list):
        ...
        if self.total_proxies == 1:
            log_step(
                "Single proxy detected - will use the same proxy for all requests",
                "warning",
            )
        elif self.total_proxies > 1:
            log_step(
                f"Multiple proxies detected ({self.total_proxies}) - will rotate proxies",
                "info",
            )
        else:
...
class ReferralClient:
    ...
    async def _get_captcha_with_retry(
        self, captcha_service, step: str = "unknown"
    ) -> Optional[str]:
        for attempt in range(1, self.max_retries + 1):
            try:
                log_step(
                    f"Getting captcha token for {step} (attempt {attempt}/{self.max_retries})...",
                    "info",
                )
                ...
                if attempt == self.max_retries:
                    log_step(
                        f"Failed to get captcha after {self.max_retries} attempts",
                        "error",
                    )
                    raise
    ...
    async def _register_with_retry(self, register_data: dict) -> Optional[dict]:
        for attempt in range(1, self.max_retries + 1):
            try:
                log_step(
                    f"Registering account (attempt {attempt}/{self.max_retries})...",
                    "info",
                )
                response = await self._make_request(
                    "POST", ApiEndpoints.Auth.REGISTER, register_data
                )

                if response.get("success"):
                    log_step(
                        f"Registration successful: {response.get('msg', 'Success')}",
                        "success",
                    )
                    return response

                log_step(
                    f"Registration failed on attempt {attempt}: {response.get('msg', 'Unknown error')}",
                    "error",
                )
                if attempt == self.max_retries:
                    return None
    ...
    def _generate_credentials(self) -> Tuple[str, str, str]:
        ...
        username = self.faker.user_name()[:15] + "".join(
    ...
    async def login(self, captcha_service) -> Optional[str]:
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                captcha_token = await self._get_captcha_with_retry(
                    captcha_service, "login"
                )
                ...
                response = await self._make_request(
                    method="POST", endpoint=ApiEndpoints.Auth.LOGIN, json_data=json_data
                )
    ...
    async def activate_account(self, access_token: str) -> Optional[dict]:
        for attempt in range(1, self.max_retries + 1):
            try:
                log_step(
                    f"Attempting account activation (attempt {attempt}/{self.max_retries})...",
                    "info",
                )
                response = await self._make_request(
                    method="POST",
                ...
                if response.get("success"):
                    log_step(
                        f"Account activation successful: {response.get('msg', 'Success')}",
                        "success",
                    )
                    return response

                log_step(
                    f"Activation failed on attempt {attempt}: {response.get('msg', 'Unknown error')}",
                    "error",
                )
                if attempt == self.max_retries:
                    return None
    ...
    async def process_referral(self, ref_code: str, captcha_service) -> Optional[Dict]:
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                if attempt == self.max_retries:
                    log_step(
                        f"Referral process failed after {self.max_retries} attempts",
                        "error",
                    )
                    return None

                log_step(
                    f"Retrying entire referral process in {delay:.2f} seconds...",
                    "info",
                )
                await asyncio.sleep(delay)
...
async def main():
    ...
    num_referrals = int(
        input(f"{Fore.GREEN}Enter number of referrals: {Style.RESET_ALL}")
    )
    ...
    service_choice = input(
        f"{Fore.GREEN}Choose captcha service (1-4): {Style.RESET_ALL}"
    )
    if service_choice == "4":
        api_key_or_url = input(
            f"{Fore.GREEN}Enter URL Endpoint for captcha service: {Style.RESET_ALL}"
        )  # No API key required for the custom service
    else:
        api_key_or_url = input(
            f"{Fore.GREEN}Enter API key for captcha service: {Style.RESET_ALL}"
        )

    use_proxies = (
        input(f"{Fore.GREEN}Use proxies? (yes/no): {Style.RESET_ALL}").lower() == "yes"
    )
    proxy_manager = None
    ...
    try:
        captcha_service = CaptchaServiceFactory.create_service(
            service_map[service_choice], api_key_or_url
        )
